...
##  Quebec Reservoir Example

**Context:**  
The Robert-Bourassa reservoir in northern Québec holds around 62 km³ of water and generates 5.6 GW via 16 turbines.  
Operators must balance **flood safety** (avoid overflow) and **energy reliability** (ensure sufficient power).  
...
where:
- \(x_t\): water level  
- \(u_t\): controlled outflow  
- \(r_t\): natural inflow (rainfall, snowmelt)

---

###  Adding Stochasticity

Inflow \(r_t\) (or noise term \(w_t\)) is uncertain and varies by season:
- **Spring:** snowmelt → long-tailed (LogNormal / Gamma)  
- **Summer:** convective storms → mixture of zero and heavy tail  
...
\]
Then estimate the empirical inflow distribution using LogNormal or mixture models.  

---

###  Advantages

- Keeps the physical model fixed → learn only the uncertainty.  
- Enforces **mass conservation**, preventing unrealistic fits.  
...
> Separate deterministic physics (known constraints)  
> from stochastic components (environmental uncertainty).  
> This yields interpretable, stable models for decision and control.
...
Previously, we assumed full observability of $\mathbf{x}_t$.  
In reality, sensors are limited, measurements noisy, and variables hidden.  
Hence we observe only **partial information** via $\mathbf{y}_t$.
...
This defines a **partially observed system**, where the hidden state evolves under control and process noise, and observations depend on it via $h_t$.

---

###  State Estimation

- If $h_t$ is invertible and noise-free → direct reconstruction.  
- With noise → **probabilistic filtering** required.

Methods: Kalman, Extended/Unscented Kalman, Particle, Neural Filters.
...
→ Classical **state-space model** solved via **Kalman Filter**, maintaining Gaussian belief over $\mathbf{x}_t$.
...
Alternative perspective: learn this kernel directly when sensors or perception pipelines are black-box.
...
Kalman filtering estimates hidden states, optimal control (LQR, MPC) compensates residuals.  
This setting combines **partial observability, stochasticity, and feedback**, bridging RL and control.

---

**Takeaway:**  
> Partial observability requires belief-based reasoning:  
> we act not on the true state but on our **belief distribution** maintained through filtering.