...
;(async () => {
  ...
  const App = {
    setup() {
      ...
      return { t, locale, switchLang, projects, links }
    },
    template: `
      <div class="relative min-h-screen">
        ...
        <nav class="max-w-6xl mx-auto flex justify-between items-center px-6 py-4">
          <div class="text-xl md:text-2xl font-bold text-sky-300 tracking-wide">Yuan Li · AI</div>
          <div class="flex items-center gap-6 text-sm">
            <a href="#" class="hover:text-sky-300">{{ t('nav.home') }}</a>
            <a href="#projects" class="hover:text-sky-300">{{ t('nav.projects') }}</a>
            <a href="#about" class="hover:text-sky-300">{{ t('nav.about') }}</a>
            <a href="#contact" class="hover:text-sky-300">{{ t('nav.contact') }}</a>
            <div class="flex gap-2">
              <button v-for="l in ['en','zh','fr']"
                :key="l"
                @click="switchLang(l)"
                :class="['px-2 py-1 rounded border transition', locale===l ? 'bg-sky-500 text-white border-sky-500' : 'bg-slate-800 border-slate-700 hover:bg-slate-700']">
                {{ l.toUpperCase() }}
              </button>
            </div>
          </div>
        ...
        <header class="max-w-6xl mx-auto px-6 mt-10 md:mt-20 text-center">
          <img src="assets/avatar.png" alt="avatar" class="mx-auto w-24 h-24 rounded-full mb-6 ring-2 ring-sky-400/60">
          <h1 class="text-3xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-fuchsia-400 to-cyan-300">
            {{ t('hero.greeting') }}
          </h1>
          <h2 class="mt-3 text-lg md:text-2xl text-sky-200">{{ t('hero.title') }}</h2>
          <p class="mt-2 md:mt-3 text-slate-300">{{ t('hero.subtitle') }}</p>
        </header>

        <section id="interests" class="max-w-6xl mx-auto px-6 mt-10">
          <h3 class="text-xl md:text-2xl font-semibold text-sky-300 mb-3">{{ t('interests.title') }}</h3>
          <p class="text-slate-300 leading-relaxed">{{ t('interests.text') }}</p>
        </section>

        <section id="projects" class="max-w-6xl mx-auto px-6 mt-12 md:mt-16">
          <h3 class="text-xl md:text-2xl font-semibold text-sky-300 mb-6">{{ t('projects.title') }}</h3>
          <div class="grid gap-6 md:grid-cols-3">
            <div v-for="p in projects" :key="p.name"
              class="bg-slate-800/80 backdrop-blur rounded-2xl overflow-hidden border border-slate-700 hover:border-sky-500/60 hover:shadow-lg hover:shadow-sky-500/20 transition">
              <img :src="p.image" class="w-full h-44 object-cover" alt="">
              <div class="p-4">
                <div class="text-base md:text-lg font-semibold text-sky-300">{{ p.name }}</div>
                <p class="mt-1 text-sm text-slate-300">{{ p.description[locale] }}</p>
                <div class="mt-3 flex flex-wrap gap-2">
                  <span v-for="tag in p.tech" :key="tag"
                    class="text-xs px-2 py-1 rounded-full bg-slate-700/70 border border-slate-600 text-slate-200">
                    {{ tag }}
                  </span>
                </div>
                <div class="mt-4">
        ...
        <section id="about" class="max-w-6xl mx-auto px-6 mt-16">
          <h3 class="text-xl md:text-2xl font-semibold text-sky-300 mb-3">{{ t('about.title') }}</h3>
          <p class="text-slate-300 leading-relaxed">{{ t('about.text') }}</p>
        </section>

        <section id="contact" class="max-w-6xl mx-auto px-6 mt-16">
          <h3 class="text-xl md:text-2xl font-semibold text-sky-300 mb-4">{{ t('contact.title') }}</h3>
          <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="text-slate-300">
              <span class="font-medium">{{ t('contact.email') }}:</span>
              <a :href="'mailto:'+links.email" class="text-sky-300 hover:underline">{{ links.email }}</a>
            </div>
